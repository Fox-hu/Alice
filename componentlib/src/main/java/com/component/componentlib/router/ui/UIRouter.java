...
public class UIRouter implements IUIRouter {
    ...
    public boolean openUri(Context context, String url, Bundle bundle, Integer requestCode) {
        ...
        if (!TextUtils.isEmpty(url)) {
            if (!url.contains("://") && (!url.startsWith("tel:") || !url.startsWith("smsto:") ||
                                         !url.startsWith("file:"))) {
                url = "http://" + url;
            }
    ...
    public boolean openUri(Context context, Uri uri, Bundle bundle, Integer requestCode) {
        for (IComponentRouter uiRouter : uiRouters) {
            try {
                if (uiRouter.verifyUri(uri) && uiRouter.openUri(context, uri, bundle,
                        requestCode)) {
                    return true;
                }
            } catch (Exception e) {
    ...
    public void registerUI(IComponentRouter router, int priority) {
        ...
        removeOldRouter(router);
        int i = 0;
        //每次插入按优先级顺序由大到小进行排序
        for (IComponentRouter uiRouter : uiRouters) {
            Integer tp = priorities.get(uiRouter);
    ...
    private void removeOldRouter(IComponentRouter router) {
    ...
    private IComponentRouter fetch(String host) {
